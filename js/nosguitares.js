...
document.addEventListener('DOMContentLoaded', () => {
    fetch('http://127.0.0.1:8000/api/guitars')
        .then(response => response.json())
        .then(data => {
            
            const baseUrl = "http://127.0.0.1:8000/uploads/images/";  // Assure-toi que c'est le bon chemin
            data.forEach(guitar => {
                const imageUrl = guitar.image ? `${baseUrl}${guitar.image}` : 'chemin/vers/image/default.jpg'; // Si `guitar.image` est undefined
                const card = `
                <div class="col-md-4 mb-4">
                    <div class="card image-card" data-guitar-id="${guitar.id}">
                        <img src="${imageUrl}" class="card-img-top" alt="${guitar.title}" style="height: 200px; object-fit: cover;">
                        ...
                        <div class="card-body">
                            <h5 class="card-title">${guitar.title}</h5>
            ...
            // Appel à la fonction pour gérer la visibilité des éléments
...
document.querySelectorAll('.btn-outline-light[data-bs-target="#EditionPhotoModal"]').forEach(button => {
    button.addEventListener('click', function() {
        ...
        document.getElementById('guitarId').value = card.getAttribute('data-guitar-id');
        ...
        document.getElementById('saveChangesBtn').onclick = function() {
            ...
            fetch(`http://127.0.0.1:8000/api/guitars/${guitarId}`, {
                method: 'PUT',
                headers: {
    ...
    document.querySelectorAll('.btn-outline-light[data-bs-target="#DeletePhotoModal"]').forEach(button => {
        button.addEventListener('click', function() {
            ...
            document.getElementById('guitarIdToDelete').value = card.getAttribute('data-guitar-id');
        });
    });
